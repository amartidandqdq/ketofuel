def test_ketosis_returns_not_started_without_keto_start_date(client):
...
def test_ketosis_returns_not_applicable_for_non_keto_diet(client, sample_profile):
...
def test_ketosis_carnivore_returns_valid_phase_with_fasting_bonus(client, sample_profile):
...
def test_ketosis_exercise_bonus_capped_at_daily_max(client, sample_profile):
...
def test_ketosis_returns_speed_tips_and_accelerators(client, sample_profile):
    client.post("/api/profile", json=sample_profile)
    r = client.get("/api/ketosis")
    data = r.json()
